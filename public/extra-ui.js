(function () {
  ...
  function ensureProfileTab() {
    ...
    if (bar && !bar.querySelector('[data-tab="profile"]')) {
      var b = document.createElement("button"); b.className = "tab"; b.dataset.tab = "profile"; b.textContent = "profile";
      bar.appendChild(b); b.onclick = function () { showProfile(); };
    }
    if (!el("view-profile")) {
  ...
  function showProfile() {
    ...
    el("view-profile").innerHTML =
      ...
      '<div class="glass card"><h2>choose official plan day</h2><p class="sub">only your 7-day list</p>' + sched + '<button class="btn full" id="pfSched" style="margin-top:12px">save schedule</button></div>';
  ...
  function signupModal() {
    ...
    el("regGo").onclick = async function () {
      ...
      try {
        ...
        m.classList.add("hidden"); toast("Account created. Login.");
  ...
  if (typeof drawChart === "function") {
    ...
    drawChart = function (canvas, rows) {
      ...
      function hit(ev) {
        ...
        var list = canvas._pts || []; if (!list.length) return;
        var i = Math.round((x - 24) / ((canvas.width - 50) / Math.max(list.length - 1, 1)));
        i = Math.max(0, Math.min(list.length - 1, i));
        var p = list[i]; var prev = i ? list[i - 1] : null;
        ...
        tip.className = "tip";
        tip.textContent = p.date + "  •  " + p.entryWeight + " kg" + (d != null ? "  •  " + (d > 0 ? "+" : "") + d + " kg" : "");
        canvas.parentNode.style.position = "relative"; canvas.parentNode.appendChild(tip);
        tip.style.left = ((ev.touches ? ev.touches[0].clientX : ev.clientX) - r.left) + "px"; tip.style.top = "8px";
      }
