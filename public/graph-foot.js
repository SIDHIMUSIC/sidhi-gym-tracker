(function () {
  ...
  function logos() {
    return '<div class="soc">' +
      '<a href="https://www.instagram.com/harryashu_/" target="_blank" rel="noopener" aria-label="Instagram">' + IG + '<span>Instagram</span></a>' +
      '<a href="https://t.me/SANATANI_BACHA" target="_blank" rel="noopener" aria-label="Telegram">' + TG + '<span>Telegram</span></a>' +
      '<a href="https://github.com/SIDHIMUSIC" target="_blank" rel="noopener" aria-label="GitHub">' + GH + '<span>GitHub</span></a></div>';
  }
  ...
  if (foot) {
    foot.innerHTML = "<b>SIDHI GYM TRACKER</b><div>Built with ❤️ by Harry</div>" + logos() + "<div>© 2026 SIDHI GYM TRACKER. All rights reserved.</div>";
  }
  ...
  function drawReal(canvas, rows) {
    ...
    var pts = (rows || []).filter(function (s) {
      return s.entryWeight != null && isFinite(Number(s.entryWeight));
    }).slice().sort(function (a, b) { return a.date.localeCompare(b.date); }).slice(-10);
    canvas._wpts = pts;
    if (!pts.length) {
  ...
  function bind(canvas) {
    ...
    function hit(ev) {
      ...
      var left = 52, right = 16;
      var i = Math.round((cx - left) / ((canvas.width - left - right) / Math.max(pts.length - 1, 1)));
      ...
      box.textContent = p.date + "  •  " + Number(p.entryWeight).toFixed(1) + " kg" +
        (p.entryTime ? "  •  " + p.entryTime : "") +
        (d != null ? "  •  " + (d > 0 ? "+" : "") + d + " kg" : "  •  first log");
