(function () {
  ...
  (function ensureUI() {
    if (!document.getElementById("run-css")) {
      ...
      st.textContent = ".head-actions{display:flex;flex-direction:column;align-items:flex-end;gap:8px}.head-btns{display:flex;gap:8px;align-items:center}.btn.run{min-height:42px;border-radius:999px;padding:8px 14px;background:linear-gradient(135deg,#63e2b3,#7ab8ff);border:0;color:#04140d;font:700 11px Comfortaa,sans-serif}.livebar{display:none;gap:6px;flex-wrap:wrap;justify-content:flex-end}.livebar.on{display:flex}.chip{padding:5px 8px;border-radius:999px;background:rgba(99,226,179,.14);font:800 10px Comfortaa,sans-serif;color:#c8ffe8}.run-ov{position:fixed;inset:0;z-index:50;display:none;overflow:auto;background:#070b12;padding:14px}.run-ov.on{display:block}.run-clock{font:800 64px Comfortaa,sans-serif;text-align:center;margin:10px 0;color:#f0c27a}.modes{display:flex;flex-wrap:wrap;gap:8px;justify-content:center;margin:12px 0}.modes button{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);color:#fff;border-radius:999px;padding:10px 12px}.modes button.on{color:#04140d;background:linear-gradient(135deg,#a8edea,#63e2b3)}.run-stats{display:grid;grid-template-columns:1fr 1fr;gap:10px}.run-stat{text-align:center;padding:14px 8px}.run-stat b{display:block;font:800 22px Comfortaa,sans-serif;color:#f0c27a}";
    ...
    if (!el("homeRun")) {
      ...
      if (tm && tm.parentNode && tm.parentNode.parentNode) {
        var row = document.createElement("div"); row.className = "kv"; row.innerHTML = "<span>\u0280\u1d1c\u0274</span><b id=\"homeRun\">\u2014</b>";
